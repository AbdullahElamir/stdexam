...
getOneItemRow : function(id,cb){
       
        mysqlMgr.connect(function (conn) {
            conn.query('select id,name,Barcode,quantity,description,idorder from item_info,order_has_item where iditem=item_info_id   and id= ?',id,function(err, result1) {
            conn.release();
          if(err) {
             util.log(err);
         } else {
  ...
  getItemInOrder  : function(limit,id,cb){
       
        mysqlMgr.connect(function (conn) {
            conn.query('select id,name,Barcode,quantity,description from item_info,order_has_item where iditem=item_info_id   and idorder= ?  ORDER BY `id` DESC limit ?,10 ; SELECT COUNT(*) as cnt  from item_info,order_has_item where iditem=item_info_id   and idorder= ?',[id,limit,id],function(err, result1) {
            conn.release();
          if(err) {
             util.log(err);
         } else {
...
getorder :function(id,cb){
 ...
 getemp :function(cb){
  ...
  getitemAll :function(cb){
  mysqlMgr.connect(function (conn) {
      conn.query('SELECT iditem,name FROM item_info,item where deleted = 1 and id_item_info_id=item_info_id',function(err, result) {
      conn.release();
    if(err) {
       util.log(err);
   } else {
 ...
 Updatequantity : function(body,cb){
    mysqlMgr.connect(function (conn) {
      conn.query('update `order_has_item` set quantity = ? WHERE `id`=?',[body.value,body.pk], function(err, result) {
        conn.release();
        if(err) {
          util.log(err);
        } else {
  ...
  UpdateDescription : function(body,cb){
    mysqlMgr.connect(function (conn) {
      conn.query('update `order_has_item` set description = ? WHERE `id`=?',[body.value,body.pk], function(err, result) {
        conn.release();
        if(err) {
          util.log(err);
        } else {
  ...
  updateIteem : function(body,cb){
    mysqlMgr.connect(function (conn) {
      conn.query('update `order_has_item` set iditem = ? WHERE `id`=?',[body.value,body.pk], function(err, result) {
        conn.release();
        if(err) {
          util.log(err);
        } else {
